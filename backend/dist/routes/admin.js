...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const totalFaculty = await db_1.default.faculty.count();
        const totalDepartments = await db_1.default.department.count();
        const totalSubjects = await db_1.default.subject.count();
        const recentNotices = await db_1.default.notice.findMany({
            take: 5,
...
router.post('/students', async (req, res) => {
    ...
    try {
        ...
        const existingRoll = await db_1.default.student.findUnique({ where: { rollNumber } });
        if (existingRoll) {
...
router.get('/faculty', async (req, res) => {
    try {
        const faculty = await db_1.default.faculty.findMany({
            include: {
                user: {
                    select: {
                        id: true,
...
router.post('/faculty', async (req, res) => {
    ...
    try {
        ...
        const existingEmp = await db_1.default.faculty.findUnique({ where: { employeeId } });
        if (existingEmp) {
            return res.status(400).json({ message: 'Employee ID already exists' });
        }
        const hashedPassword = await bcryptjs_1.default.hash(password, 10);
        const faculty = await db_1.default.$transaction(async (tx) => {
            const user = await tx.user.create({
                data: {
                    name,
                    email,
...
router.get('/departments', async (req, res) => {
    try {
        const depts = await db_1.default.department.findMany({
            include: {
                _count: {
                    select: {
                        students: true,
...
router.post('/departments', async (req, res) => {
    ...
    try {
        const existingCode = await db_1.default.department.findUnique({ where: { code } });
        if (existingCode) {
            return res.status(400).json({ message: 'Department code already exists' });
        }
        const dept = await db_1.default.department.create({
            data: { name, code },
        });
...
router.get('/semesters', async (req, res) => {
    try {
        const semesters = await db_1.default.semester.findMany({
            orderBy: { number: 'asc' },
        });
...
router.get('/subjects', async (req, res) => {
    try {
        const departmentId = req.query.departmentId ? parseInt(req.query.departmentId, 10) : undefined;
...
router.post('/subjects', async (req, res) => {
    try {
        ...
        const existingCode = await db_1.default.subject.findUnique({ where: { code } });
        if (existingCode) {
        ...
        const semester = await db_1.default.semester.findUnique({ where: { id: semesterId } });
        if (semester) {
            const students = await db_1.default.student.findMany({
                where: {
                    departmentId,
            ...
            if (students.length > 0) {
                await db_1.default.enrollment.createMany({
...
router.post('/timetable', async (req, res) => {
    ...
    try {
        ...
        let timetable = await db_1.default.timetable.findFirst({
            where: {
                departmentId: Number(departmentId),
        ...
        if (!timetable) {
            timetable = await db_1.default.timetable.create({
                data: {
                    departmentId: Number(departmentId),
                    semesterId: Number(semesterId),
                },
            ...
            await db_1.default.timetableSlot.deleteMany({
        ...
        await db_1.default.timetableSlot.createMany({
            data: slotsData,
        });
